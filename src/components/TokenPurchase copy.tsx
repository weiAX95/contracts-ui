...
const TokenPurchase = () => {
    ...
    const [rate, setRate] = useState(1000); // 假设1 ETH = 1000 代币
    ...
    const handlePurchase = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // 这里需要替换为你的代币合约地址
            const contractAddress = "0x5FbDB2315678afecb367f032d93F642f64180aa3"; // 你的代币合约地址
            const ethAmountInWei = '0x' + (Number(formData.ethAmount) * 1e18).toString(16); // 转换为wei

            // 构建购买交易参数
            const transactionParameters = {
                to: contractAddress,
                from: accounts[0],
                value: ethAmountInWei, // ETH 以 wei 为单位
                data: '0x' // 假设合约方法是 `buyWithETH`，你需要根据你的合约方法替换 data 字段
            };
